...
interface WorkflowState {
    ...
    createWorkflow: (buId: string, name: string, flowType: string, description?: string) => Promise<Workflow | null>;
...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
    ...
    createWorkflow: async (buId: string, name: string, flowType: string, description?: string) => {
        set({ loading: true, error: null });
        try {
            const { createWorkflow: apiCreateWorkflow } = await import('../../../shared/lib/api');
            const newWorkflow = await apiCreateWorkflow(buId, name, flowType, description);
